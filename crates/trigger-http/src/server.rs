...
impl<F: RuntimeFactors> HttpServer<F> {
    ...
    ///
    /// Forces each worker to start and instantiate the WASM component eagerly so
    /// the first real request doesn't pay the ~263ms cold-start penalty from
    /// mprotect + CoW image mapping + data-segment memcpy.
            ...
            if let HandlerType::Wasi0_3(proxy_handler) = handler_type {
                ...
                proxy_handler.spawn(
                    ...
                    Box::new(move |_store, _proxy| {
                        Box::pin(async move {
                            // Signal that the Worker started and the WASM instance is live.
