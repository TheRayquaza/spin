...
impl<T: RuntimeFactors, U: Send + 'static> FactorsExecutorApp<T, U> {
    ...
    ///
    /// Errors from individual component initializations are returned immediately.
    /// Call sites that should not abort startup on prewarm failure may choose to
    /// log and discard the error.
